// Edge Function: envia OTP por SMS ou WhatsApp usando Twilio Verify.
// O proprio Twilio gera/gerencia o codigo. Requer JWT valido.
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { channel = 'sms' } = await req.json().catch(() => ({ channel: 'sms' }))
    ...
    const resp = await fetch(`https://verify.twilio.com/v2/Services/${TW_VERIFY}/Verifications`, {
      method: 'POST',
      headers: { Authorization: `Basic ${basic}`, 'Content-Type': 'application/x-www-form-urlencoded' },
      body: new URLSearchParams({ To: phone, Channel: ch }),
    })
    const data = await resp.json()
    if (!resp.ok) return json({ success: false, error: 'twilio_error', detail: data?.message ?? data })

    return json({ success: true, to: maskPhone(phone), channel: ch, status: data.status })
  } catch (e) {

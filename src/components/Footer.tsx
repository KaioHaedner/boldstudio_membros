// Footer padrao do site, usado em TODAS as paginas fora da home institucional
// (login, cadastro, admin, checkout, 404, Em Breve, politicas/RecIA, etc).
// Mesmo visual e comportamento do footer da home (components/home/Footer.tsx),
// porem autossuficiente: nao depende do I18nProvider e os links do mapa do site
// apontam pra home + ancora (funcionam a partir de qualquer pagina).
...
// videos (#cases) rotulada como "Reels". `key` mapeia a traducao (t.nav);
// `label` e o fallback PT quando nao ha idioma no contexto.
const NAV_LINKS = [
  { href: `${HOME}#home`, key: 'home', label: 'Home' },
  { href: `${HOME}#sobre`, key: 'sobre', label: 'Sobre' },
  { href: `${HOME}#servicos`, key: 'servicos', label: 'Serviços' },
  { href: `${HOME}#crew`, key: 'crew', label: 'Crew' },
  { href: `${HOME}#cases`, key: 'reels', label: 'Reels' },
  { href: `${HOME}#processo`, key: 'processo', label: 'Processo' },
  { href: `${HOME}#clientes`, key: 'clientes', label: 'Clientes' },
  { href: `${HOME}#contato`, key: 'contato', label: 'Contato' },
] as const

const LEGAL_LINKS = [
  { href: '/termos', key: 'termos', label: 'Termos de uso' },
  { href: '/privacidade', key: 'privacidade', label: 'Privacidade' },
  { href: '/cookies', key: 'cookies', label: 'Cookies' },
  { href: '/uso-da-ia', key: 'ia', label: 'Uso da IA (RecIA)' },
] as const

// Icones de marca reais (mesmos SVGs do footer da home).
const SOCIALS = [
  {
    name: 'Instagram',
  ...
  {
    name: 'E-mail',
    href: 'mailto:bold@boldstudiobrasil.com',
    external: false,
...
export function Footer() {
  ...
  // Quando ha idioma no contexto (ex.: pagina de projeto do cliente), traduz.
    ...
    <footer className="relative z-10 border-t border-white/10 bg-bold-black px-6 py-14 text-bold-white">
      <div className="mx-auto flex max-w-5xl flex-col gap-10 md:flex-row md:justify-between">
        <div className="flex flex-col gap-4">
          ...
          />
          <p className="max-w-xs text-sm font-semibold text-bold-white">{tagline}</p>
        ...
        <div className="flex flex-col gap-3">
          <p className="text-[1.3125rem] font-bold leading-tight tracking-wider text-bold-yellow">{sitemapTitle}</p>
          <ul className="flex flex-col gap-2 text-sm font-semibold text-bold-white">
            {NAV_LINKS.map((link) => (
              <li key={link.href}>
                <a href={link.href} className="transition-colors hover:text-bold-yellow">
                  {t ? t.nav[link.key] : link.label}
                </a>
              </li>
            ))}
          </ul>
        ...
        <div className="flex flex-col gap-3">
          <p className="text-[1.3125rem] font-bold leading-tight tracking-wider text-bold-yellow">{essentialTitle}</p>
          <ul className="flex flex-col gap-2 text-sm font-semibold text-bold-white">
            {LEGAL_LINKS.map((link) => (
              <li key={link.href}>
                <a href={link.href} className="transition-colors hover:text-bold-yellow">
                  {t ? t.footer.legal[link.key] : link.label}
                </a>
              </li>
            ))}
          </ul>
      ...
      <div className="mx-auto mt-12 flex max-w-5xl flex-col items-center gap-3 border-t border-white/5 pt-6 text-center">
        ...
        <p className="text-xs text-bold-white/40">
          © {new Date().getFullYear()} Bold Studio Brasil. {rights}
        </p>
      </div>
    </footer>
  )
